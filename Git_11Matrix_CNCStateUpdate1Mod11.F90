...
! local pointers to implicit in/out arrays
   real(r8), pointer :: cpool(:)              ! (gC/m2) temporary photosynthate
!C pool
...
!(1=woody, 0=not woody)
!========================chen matrix test  keep the input update here==========================
   real(r8), pointer :: cwdc_to_litr2c(:)       ! decomp. of coarse woody debris
!C to litter 2 C (gC/m2/s)
   real(r8), pointer :: cwdc_to_litr3c(:)       ! decomp. of coarse woody debris
!C to litter 3 C (gC/m2/s)
!==============================================================================================
   integer , pointer :: harvdate(:)             ! harvest date
   real(r8), pointer :: xsmrpool_to_atm(:)      ! excess MR pool harvest
!mortality (gC/m2/s)



!==================chen matrix test2  keep the input update here ==============================
...
!C (gC/m2/s)
!=================chen matrix test  update in Decomp Module=============================
    real(r8), pointer :: litr1_hr(:)             ! het. resp. from litter 1 C
!(gC/m2/s)
    real(r8), pointer :: litr1c_to_soil1c(:)     ! decomp. of litter 1 C to SOM
!1 C (gC/m2/s)
    real(r8), pointer :: litr2_hr(:)             ! het. resp. from litter 2 C
!(gC/m2/s)
    real(r8), pointer :: litr2c_to_soil2c(:)     ! decomp. of litter 2 C to SOM
!2 C (gC/m2/s)
    real(r8), pointer :: litr3_hr(:)             ! het. resp. from litter 3 C
!(gC/m2/s)
    real(r8), pointer :: litr3c_to_soil3c(:)     ! decomp. of litter 3 C to SOM
!3 C (gC/m2/s)
    real(r8), pointer :: soil1_hr(:)             ! het. resp. from SOM 1 C
!(gC/m2/s)
    real(r8), pointer :: soil1c_to_soil2c(:)     ! decomp. of SOM 1 C to SOM 2 C
!(gC/m2/s)
    real(r8), pointer :: soil2_hr(:)             ! het. resp. from SOM 2 C
!(gC/m2/s)
    real(r8), pointer :: soil2c_to_soil3c(:)     ! decomp. of SOM 2 C to SOM 3 C
!(gC/m2/s)
    real(r8), pointer :: soil3_hr(:)             ! het. resp. from SOM 3 C
!(gC/m2/s)
    real(r8), pointer :: soil3c_to_soil4c(:)     ! decomp. of SOM 3 C to SOM 4 C
!(gC/m2/s)
    real(r8), pointer :: soil4_hr(:)             ! het. resp. from SOM 4 C
!(gC/m2/s)
!==============================================================================================
   real(r8), pointer :: col_ctrunc(:)           ! (gC/m2) column-level sink for
!C truncation
   ...
   real(r8), pointer :: cpool_livestem_gr(:)         ! live stem growth
!respiration (gC/m2/s)
   real(r8), pointer :: cpool_livestem_storage_gr(:) ! live stem growth
!respiration to storage (gC/m2/s)
   real(r8), pointer :: transfer_deadcroot_gr(:)     ! dead coarse root growth
!respiration from storage (gC/m2/s)
   real(r8), pointer :: transfer_deadstem_gr(:)      ! dead stem growth
!respiration from storage (gC/m2/s)
   real(r8), pointer :: transfer_froot_gr(:)         ! fine root  growth
!respiration from storage (gC/m2/s)
   real(r8), pointer :: transfer_leaf_gr(:)          ! leaf growth respiration
!from storage (gC/m2/s)
   real(r8), pointer :: transfer_livecroot_gr(:)     ! live coarse root growth
!respiration from storage (gC/m2/s)
   real(r8), pointer :: transfer_livestem_gr(:)      ! live stem growth
!respiration from storage (gC/m2/s)
   real(r8), pointer :: cpool_to_grainc(:)           ! allocation to grain C
!(gC/m2/s)
   real(r8), pointer :: cpool_to_grainc_storage(:)   ! allocation to grain C
!storage (gC/m2/s)
   real(r8), pointer :: grainc_storage_to_xfer(:)    ! grain C shift storage to
!transfer (gC/m2/s)
   real(r8), pointer :: livestemc_to_litter(:)       ! live stem C litterfall
!(gC/m2/s)
   real(r8), pointer :: grainc_to_food(:)            ! grain C to food (gC/m2/s)
   real(r8), pointer :: grainc_xfer_to_grainc(:)     ! grain C growth from
!storage (gC/m2/s)
   real(r8), pointer :: cpool_grain_gr(:)            ! grain growth respiration
!(gC/m2/s)
   real(r8), pointer :: cpool_grain_storage_gr(:)    ! grain growth respiration
!to storage (gC/m2/s)
   real(r8), pointer :: transfer_grain_gr(:)         ! grain growth respiration
!from storage (gC/m2/s)
!
! local pointers to implicit in/out arrays
   ...
   real(r8), pointer :: soil1c(:)             ! (gC/m2) soil organic matter C
!(fast pool)
   real(r8), pointer :: soil2c(:)             ! (gC/m2) soil organic matter C
!(medium pool)
   real(r8), pointer :: soil3c(:)             ! (gC/m2) soil organic matter C
!(slow pool)
...
!C pool
   ...
   real(r8), pointer :: deadcrootc_storage(:) ! (gC/m2) dead coarse root C
!storage
   real(r8), pointer :: deadcrootc_xfer(:)    ! (gC/m2) dead coarse root C
!transfer
   ...
   real(r8), pointer :: gresp_storage(:)      ! (gC/m2) growth respiration
!storage
   real(r8), pointer :: gresp_xfer(:)         ! (gC/m2) growth respiration
!transfer
   real(r8), pointer :: leafc(:)              ! (gC/m2) leaf C
   real(r8), pointer :: leafc_storage(:)      ! (gC/m2) leaf C storage
   real(r8), pointer :: leafc_xfer(:)         ! (gC/m2) leaf C transfer
   real(r8), pointer :: livecrootc(:)         ! (gC/m2) live coarse root C
   real(r8), pointer :: livecrootc_storage(:) ! (gC/m2) live coarse root C
!storage
   real(r8), pointer :: livecrootc_xfer(:)    ! (gC/m2) live coarse root C
!transfer
...
!-----------------------------------------------------------------------
    ...
    !chen: print test for site study
    !print *, "matrix works"
    ...
    !chen: print test for site study       
            !  print *,"cwdc11 = ",cwdc(c)
            !  print *,"litr1c11 = ",litr1c(c)
            !  print *,"litr2c11 = ",litr2c(c)
            !  print *,"litr3c11 = ",litr3c(c)
            !  print *,"soil1c11 = ",soil1c(c)
            !  print *,"soil2c11 = ",soil2c(c)
            !  print *,"soil3c11 = ",soil3c(c)
            !  print *,"soil4c11 = ",soil4c(c)
            
!======chen matrix test1 move the updates to Decomp Module========        
...
!==================================================================== 
!=chen matrix test reactive cwdc updates here for 7pool module=== 
...
cpool_to_livestemc_storage(p)*dt
           cpool(p)               = cpool(p)              -&
cpool_to_grainc(p)*dt
           grainc(p)              = grainc(p)             +&
cpool_to_grainc(p)*dt
           cpool(p)               = cpool(p)              -&
cpool_to_grainc_storage(p)*dt
           grainc_storage(p)      = grainc_storage(p)     +&
cpool_to_grainc_storage(p)*dt
       end if
...
gresp_storage_to_xfer(p)*dt
           gresp_xfer(p)         = gresp_xfer(p)      +&
gresp_storage_to_xfer(p)*dt
       end if
       if (ivt(p) >= npcropmin) then ! skip 2 generic crops
           ! lines here for consistency; the transfer terms are zero
...
livestemc_storage_to_xfer(p)*dt
           grainc_storage(p)     = grainc_storage(p)    -&
grainc_storage_to_xfer(p)*dt
           grainc_xfer(p)        = grainc_xfer(p)       +&
grainc_storage_to_xfer(p)*dt
       end if
