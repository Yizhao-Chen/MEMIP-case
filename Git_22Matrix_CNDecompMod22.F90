...
! !USES:
   ...
   use surfrdMod, only:crop_prog   !!!! add the use for matrix
   implicit none
   save
   private
...
! !ARGUMENTS:
   ...
   integer, intent(in) :: num_pcropp      ! number of pfts in prognostic crop
!filter
!
! !CALLED FROM:
...
!
   ! column level
!==========chen:matrix test moved from CNupdate1==========================
 !  real(r8), pointer :: grainc_to_litr1c(:)     ! grain C litterfall to litter
 !  1 C (gC/m2/s)
 !  real(r8), pointer :: grainc_to_litr2c(:)     ! grain C litterfall to litter
 !  2 C (gC/m2/s)
 !  real(r8), pointer :: grainc_to_litr3c(:)     ! grain C litterfall to litter
 !  3 C (gC/m2/s)
 !  real(r8), pointer :: livestemc_to_litr1c(:)  ! livestem C litterfall to
 !  litter 1 C (gC/m2/s)
 !  real(r8), pointer :: livestemc_to_litr2c(:)  ! livestem C litterfall to
 !  litter 2 C (gC/m2/s)
 !  real(r8), pointer :: livestemc_to_litr3c(:)  ! livestem C litterfall to
 !  litter 3 C (gC/m2/s)
 !  real(r8), pointer :: frootc_to_litr1c(:)     ! fine root C litterfall to
 !  litter 1 C (gC/m2/s)
 !  real(r8), pointer :: frootc_to_litr2c(:)     ! fine root C litterfall to
 !  litter 2 C (gC/m2/s)
 !  real(r8), pointer :: frootc_to_litr3c(:)     ! fine root C litterfall to
 !  litter 3 C (gC/m2/s)
 !  real(r8), pointer :: leafc_to_litr1c(:)      ! leaf C litterfall to litter 1
 !  C (gC/m2/s)
 !  real(r8), pointer :: leafc_to_litr2c(:)      ! leaf C litterfall to litter 2
 !  C (gC/m2/s)
 !  real(r8), pointer :: leafc_to_litr3c(:)      ! leaf C litterfall to litter 3
 !  C (gC/m2/s)
!==================================================================================
...
!(-nlevsno+1:nlevgrnd)
   real(r8), pointer :: psisat(:,:)      ! soil water potential at saturation
!for CN code (MPa)
   real(r8), pointer :: soilpsi(:,:)     ! soil water potential in each soil
!layer (MPa)
   real(r8), pointer :: dz(:,:)          ! soil layer thickness (m)


!chen:matrix test move to in/out scalars
   ...
   !real(r8), pointer :: soil1c(:)        ! (gC/m2) soil organic matter C (fast
   !pool)
   !real(r8), pointer :: soil2c(:)        ! (gC/m2) soil organic matter C
   !(medium pool)
   !real(r8), pointer :: soil3c(:)        ! (gC/m2) soil organic matter C (slow
   !pool)
   !real(r8), pointer :: soil4c(:)        ! (gC/m2) soil organic matter C
   !(slowest pool)
...
!chen matrix test matrix test 1 add c input scalars
!   real(r8), pointer :: grainc_to_litr1c(:)     ! grain C litterfall to litter 1
!C (gC/m2/s)
!   real(r8), pointer :: grainc_to_litr2c(:)     ! grain C litterfall to litter 2
!C (gC/m2/s)
 !  real(r8), pointer :: grainc_to_litr3c(:)     ! grain C litterfall to litter 3
!C (gC/m2/s)
!   real(r8), pointer :: livestemc_to_litr1c(:)  ! livestem C litterfall to
!litter 1 C (gC/m2/s)
 !  real(r8), pointer :: livestemc_to_litr2c(:)  ! livestem C litterfall to
...
!   real(r8), pointer :: leafc_to_litr2c(:)      ! leaf C litterfall to litter 2
!C (gC/m2/s)
 !  real(r8), pointer :: leafc_to_litr3c(:)      !z leaf C litterfall to litter 3
!C (gC/m2/s)
 !  real(r8), pointer :: dwt_livecrootc_to_cwdc(:)
 ! real(r8), pointer :: dwt_deadcrootc_to_cwdc(:)
 
!chen matrix test added to the module from Update1Mod   
...
!chen: matrix test move the C pool update from in scalars to in/out scalars
   real(r8), pointer :: cwdc(:)          ! (gC/m2) coarse woody debris C
   real(r8), pointer :: litr1c(:)        ! (gC/m2) litter labile C
   real(r8), pointer :: litr2c(:)        ! (gC/m2) litter cellulose C
   real(r8), pointer :: litr3c(:)        ! (gC/m2) litter lignin C
   real(r8), pointer :: soil1c(:)        ! (gC/m2) soil organic matter C (fast
!pool)
   real(r8), pointer :: soil2c(:)        ! (gC/m2) soil organic matter C (medium
!pool)
   real(r8), pointer :: soil3c(:)        ! (gC/m2) soil organic matter C (slow
!pool)
   real(r8), pointer :: soil4c(:)        ! (gC/m2) soil organic matter C
...
!chen matrix test matrix variables
   real(r8),dimension(7,7)      ::a_matrix
   real(r8),dimension(7,7)      ::kk_matrix
!chen matrix test nitrogen matrix not used yet
  ! real(r8),dimension(8,8)      ::na_matrix
  ! real(r8),dimension(8,8)      ::nk_matrix
  ! real(r8):: cn_cwd
  ! real(r8):: rate1
  ! real(r8):: rate2
  ! real(r8):: rate3

   real(r8),dimension(7,1)      ::c_matrix_current_1
   real(r8),dimension(7,1)      ::c_matrix_next_1
   real(r8),dimension(7,1)      ::c_matrix_in_1
...
!-----------------------------------------------------------------------
   ...
   itypelun              => lun%itype
!===========chen: matrix test added input scalars=================
   !cwdc_to_litr2c                 => ccf%cwdc_to_litr2c
   !cwdc_to_litr3c                 => ccf%cwdc_to_litr3c
...
!====================================================================
   ...
   !chen: print test for site simulation
   !print *,"matrix_works"
   ...
   do j = 1,nlevdecomp
      do fc = 1,num_soilc
         c = filter_soilc(fc)
         t_scalar(c)=t_scalar(c) +&
(1.5**((t_soisno(c,j)-(SHR_CONST_TKFRZ+25._r8))/10._r8))*fr(c,j)
      end do
   end do
...
!===========chen matrix_test matrix initialize=============
      kk_matrix = 0.0
      c_matrix_in_1 = 0.0
      a_matrix = 0.0
     ! na_matrix = 0.0 
...
!===========chen matrix test a_matrix======================      
        !C matrix    
              !a_matrix(3,1) = cwd_fcel     !cwdc_to_litr2c
              !a_matrix(4,1) = cwd_flig        !cwdc_to_litr3c
              a_matrix(4,1) = 1.0 - rf_l1s1   !litr1c_to_soil1c
              a_matrix(5,2) = 1.0 - rf_l2s2   !litr2c_to_soil2c
              a_matrix(6,3) = 1.0 - rf_l3s3   !litr3c_to_soil3c
              a_matrix(5,4) = 1.0 - rf_s1s2   !soil1c_to_soil2c
              a_matrix(6,5) = 1.0 - rf_s2s3   !soil2c_to_soil3c
              a_matrix(7,6) = 1.0 - rf_s3s4   !soil3c_to_soil4c
              

              
          !chen: N matrix not used yet      
...
!===============chen matrix representation of cwdc================
      ! CWD fragmentation -> litter pools
      !cwdc_loss = cwdc(c) * kk_matrix(1,1)
...
!=================================================================
      ...
      !if(cwdc(c) > 0._r8) then
      !kk_matrix(1,1) = cwdc_loss /cwdc(c)
      !else
     ! kk_matrix(1,1) = 0.0
      !end if
      ...
      if (soil1c(c) > 0._r8) then
         ...
         pmnf_s1s2(c) = (psoil1c_loss(c) * (1.0_r8 - rf_s1s2 -&
(cn_s2/cn_s1)))/cn_s2
      end if
      ...
      if (soil2c(c) > 0._r8) then
         ...
         pmnf_s2s3(c) = (psoil2c_loss(c) * (1.0_r8 - rf_s2s3 -&
(cn_s3/cn_s2)))/cn_s3
      end if
      ...
      if (soil3c(c) > 0._r8) then
         ...
         pmnf_s3s4(c) = (psoil3c_loss(c) * (1.0_r8 - rf_s3s4 -&
(cn_s4/cn_s3)))/cn_s4
      end if
 ...
 do fc = 1,num_soilc
      ...
      if (litr1c(c) > 0._r8) then
         if (pmnf_l1s1(c) > 0._r8) then
          ...
          !litr1_hr(c) = litr1c(c) *&
          !kk_matrix(2,2)* (1._r8 - a_matrix(5,2))
          !litr1c_to_soil1c(c) = litr1c(c) *&
          !kk_matrix(2,2)* a_matrix(5,2)
          litr1_hr(c) = rf_l1s1 * plitr1c_loss(c)
          litr1c_to_soil1c(c) = (1._r8 - rf_l1s1) * plitr1c_loss(c)
          kk_matrix(1,1) = plitr1c_loss(c)/litr1c(c)
      ...
      if (litr2c(c) > 0._r8) then
         ...
         !litr2_hr(c) = litr2c(c) *&
         !kk_matrix(3,3)* (1._r8 - a_matrix(6,3))
         !litr2c_to_soil2c(c) = litr2c(c) *&
         !kk_matrix(3,3)* a_matrix(6,3)
         litr2_hr(c) = rf_l2s2 * plitr2c_loss(c)
         litr2c_to_soil2c(c) = (1._r8 - rf_l2s2) * plitr2c_loss(c)
         kk_matrix(2,2) =  plitr2c_loss(c)/litr2c(c)
      ...
      if (litr3c(c) > 0._r8) then
         if (pmnf_l3s3(c) > 0._r8) then
          ...
          !litr3_hr(c) = litr3c(c) *&
          !kk_matrix(4,4)* (1._r8 -a_matrix(7,4))
          !litr3c_to_soil3c(c) = litr3c(c) *&
          !kk_matrix(4,4)* a_matrix(7,4)
         litr3_hr(c) = rf_l3s3 * plitr3c_loss(c)
         litr3c_to_soil3c(c) = (1._r8 - rf_l3s3) * plitr3c_loss(c)
         kk_matrix(3,3) =  plitr3c_loss(c)/litr3c(c)
      ...
      if (soil1c(c) > 0._r8) then
         ...
         kk_matrix(4,4) =  psoil1c_loss(c)/soil1c(c)
         !     soil1_hr(c) = soil1c(c) *&
         !     kk_matrix(5,5)* (1._r8 - a_matrix(6,5))
         !     soil1c_to_soil2c(c) = soil1c(c) *&
         !     kk_matrix(5,5)* a_matrix(6,5)
      ...
      if (soil2c(c) > 0._r8) then
         ...
         !     soil2_hr(c) = soil2c(c) *&
         !     kk_matrix(6,6)* (1._r8 - a_matrix(7,6))
         !     soil2c_to_soil3c(c) = soil2c(c) *&
         !     kk_matrix(6,6)* a_matrix(7,6)
         soil2_hr(c) = rf_s2s3 * psoil2c_loss(c)
         soil2c_to_soil3c(c) = (1._r8 - rf_s2s3) * psoil2c_loss(c)
         kk_matrix(5,5)=psoil2c_loss(c)/soil2c(c)
      ...
      if (soil3c(c) > 0._r8) then
         ...
         !   kk_matrix(7,7) = ck_s3 /dt * fpi(c)
           ...
           !   soil3_hr(c) = soil3c(c) *&
           !   kk_matrix(7,7)* (1._r8 - a_matrix(8,7))
           !   soil3c_to_soil4c(c) = soil3c(c) *&
           !   kk_matrix(7,7)* a_matrix(8,7)
         soil3_hr(c) = rf_s3s4 * psoil3c_loss(c)
         soil3c_to_soil4c(c) = (1._r8 - rf_s3s4) * psoil3c_loss(c)
         kk_matrix(6,6) = psoil3c_loss(c)/soil3c(c)
      ...
      if (soil4c(c) > 0._r8) then
         ...
         kk_matrix(7,7) = psoil4c_loss(c)/soil4c(c)
...
!=============chen matrix test c pool updates=====================
              !c_matrix_current_1(1,1) = cwdc(c) 
              c_matrix_current_1(1,1) = litr1c(c)
              c_matrix_current_1(2,1) = litr2c(c)
              c_matrix_current_1(3,1) = litr3c(c)
              c_matrix_current_1(4,1) = soil1c(c)
              c_matrix_current_1(5,1) = soil2c(c)
              c_matrix_current_1(6,1) = soil3c(c)
              c_matrix_current_1(7,1) = soil4c(c)
                            
              !c_matrix_in_1(1,1)    = dwt_livecrootc_to_cwdc(c)*dt +&
!dwt_deadcrootc_to_cwdc(c)*dt
 !             c_matrix_in_1(2,1)    = leafc_to_litr1c(c)*dt +&
!frootc_to_litr1c(c)*dt + dwt_frootc_to_litr1c(c)*dt
 !             c_matrix_in_1(3,1)    = leafc_to_litr2c(c)*dt +&
!frootc_to_litr2c(c)*dt  + dwt_frootc_to_litr2c(c)*dt
 !             c_matrix_in_1(4,1)    = leafc_to_litr3c(c)*dt +&
!frootc_to_litr3c(c)*dt  +  dwt_frootc_to_litr3c(c)*dt

 !       if ( crop_prog )then
 !       c_matrix_in_1(2,1)= c_matrix_in_1(2,1)+ livestemc_to_litr1c(c)*dt
 !       c_matrix_in_1(2,1)= c_matrix_in_1(2,1)+ grainc_to_litr1c(c)*dt
 !       c_matrix_in_1(3,1)= c_matrix_in_1(3,1)+ livestemc_to_litr2c(c)*dt
 !       c_matrix_in_1(3,1)= c_matrix_in_1(3,1)+ grainc_to_litr2c(c)*dt
 !       c_matrix_in_1(4,1)= c_matrix_in_1(4,1)+ livestemc_to_litr3c(c)*dt
 !       c_matrix_in_1(4,1)= c_matrix_in_1(4,1)+ grainc_to_litr3c(c)*dt
 !       end if


              c_matrix_next_1(:,:) =  c_matrix_current_1 + &
                 matmul(matmul(a_matrix,kk_matrix),c_matrix_current_1)*dt
                litr1c(c) =  c_matrix_next_1(1,1)
                litr2c(c) = c_matrix_next_1(2,1)
                litr3c(c) = c_matrix_next_1(3,1)
                soil1c(c) = c_matrix_next_1(4,1)
                soil2c(c) = c_matrix_next_1(5,1)
                soil3c(c) = c_matrix_next_1(6,1)
                soil4c(c) =  c_matrix_next_1(7,1)
              
             ! if (cwdc(c) > 0._r8) then
              !cwdc(c) = c_matrix_next_1(1,1) 
             ! else 
             ! cwdc(c) = c_matrix_current_1(1,1)    
             ! end if
              
             ! if (litr1c(c) > 0._r8) then
             ! litr1c(c) =  c_matrix_next_1(1,1)
             ! else
             ! litr1c(c) = c_matrix_current_1(2,1)
             ! end if
              
             ! if (litr2c(c) > 0._r8) then
             ! litr2c(c) = c_matrix_next_1(2,1)
             ! else
             ! litr2c(c) = c_matrix_current_1(3,1) 
             ! end if
              
             ! if (litr3c(c) > 0._r8) then
             ! litr3c(c) = c_matrix_next_1(3,1)
             ! else
             ! litr3c(c) = c_matrix_current_1(4,1)
             ! end if
              
             ! if (soil1c(c) > 0._r8) then
             ! soil1c(c) = c_matrix_next_1(4,1)
             ! else
             ! soil1c(c) = c_matrix_current_1(5,1)
             ! end if
              
             ! if (soil2c(c) > 0._r8) then
              !soil2c(c) = c_matrix_next_1(5,1)
             ! else
             ! soil2c(c) = c_matrix_current_1(6,1)    
             ! end if
              
             ! if (soil3c(c) > 0._r8) then
            !  soil3c(c) = c_matrix_next_1(6,1)
             ! else
             ! soil3c(c) = c_matrix_current_1(7,1) 
             ! end if
              ...
              !end if
              !print *,"cwdc = ",cwdc(c)
              !print *,"litr1c = ",litr1c(c)
              !print *,"litr2c = ",litr2c(c)
              !print *,"litr3c = ",litr3c(c)
              !print *,"soil1c = ",soil1c(c)
              !print *,"soil2c = ",soil2c(c)
              !print *,"soil3c = ",soil3c(c)
              !print *,"soil4c = ",soil4c(c)
              
              
          !    print *, "cwdc_to_litr2c = ",cwdc_to_litr2c(c)
          !    print *, "cwdc_to_litr3c = ",cwdc_to_litr3c(c)
          !    print *, "litr1_hr = ",litr1_hr(c)
          !    print *, "litr1c_to_soil1c = ",litr1c_to_soil1c(c)
          !    print *, "litr2_hr = ",litr2_hr(c)
          !    print *, "litr2c_to_soil2c = ",litr2c_to_soil2c(c)
          !    print *, "litr3_hr = ",litr3_hr(c)
          !    print *, "litr3c_to_soil3c = ",litr3c_to_soil3c(c)
          !    print *, "soil1_hr = ",soil1_hr(c)
          !    print *, "soil1c_to_soil2c = ",soil1c_to_soil2c(c)
          !    print *, "soil2_hr = ",soil2_hr(c)
          !    print *, "soil2c_to_soil3c = ",soil2c_to_soil3c(c)
          !    print *, "soil3_hr = ",soil3_hr(c)
          !!    print *, "soil3c_to_soil4c = ",soil3c_to_soil4c(c)
          !    print *, "soil4_hr = ",soil4_hr(c)
